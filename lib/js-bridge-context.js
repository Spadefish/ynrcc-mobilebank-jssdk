  ...
  if (runNative) {
    ...
    if (device.isIpad || device.isIphone || device.isIpod) {
      if (!global.webkit || !global.webkit.messageHandlers) {
        _errorHandler(`浏览器中不支持global.webkit或者global.webkit.messageHandlers`)
        throw new Error(`浏览器中不支持global.webkit或者global.webkit.messageHandlers`)
      }
      _jsContext = global.webkit.messageHandlers[name]
      if (!is('UserMessageHandler', _jsContext)) {
        _errorHandler(`IOS OS浏览器中没有注入需要的客户端上下文对象：${name}`)
      ...
      if (!is('Object', _jsContext)) {
        _errorHandler(`Android OS 浏览器中没有注入需要的客户端上下文对象：${name}`)
        throw new Error(`Android OS 浏览器中没有注入需要的客户端上下文对象：${name}`)
      }
    } else {
      _errorHandler(`暂不支持此运行环境`)
