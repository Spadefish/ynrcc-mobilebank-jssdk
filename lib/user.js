    ...
    return new Promise((resolve, reject) => {
      window.getUserInfoResult = (res) => {
        ...
        if (temp[dict.statusCodeKey] === dict.statusCodeSuccess) {
          ...
          temp.ReturnMessage = base64.decode(temp[dict.msgKey])
          if(temp.ReturnCode === 'role.invalid_user' || temp.ReturnCode === 'validation.user.force.logout.exception') {
            temp.ReturnCode = '444444'
            temp.ReturnMessage = '用户未登录'
          }
          reject(temp)
        }
      }
